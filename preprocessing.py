...
class FeatureStatistics:
    def __init__(self):
        ...
        feature_dict_list = [
            # Word/Tag core features (f100-f107)
            "f100",  # (word, tag)
            "f101",  # suffixes
            "f102",  # prefixes
            "f103",  # tag trigram
            ...
            # Orthographic features (f108-f113)
            "f108",  # has_digit
            "f109",  # is_numeric
            "f110",  # has_upper
            "f112",  # all_caps
            "f113",  # num-noun pattern

            # Capitalization features (f114-f115)
            "f114",  # capital_start
            "f115",  # initcap_first
            ...
            # Neighbor affixes (f304-f307)
            "f304",  # prev word suffix
            "f305",  # prev word prefix
            "f306",  # next word suffix
            "f307",  # next word prefix

            # Position boolean flags (f309-f311)
            "f309",  # is-second?
            "f310",  # is-last?
            "f311",  # is-middle?
    ...
    def get_word_tag_pair_count(self, file_path):
        ...
        # count actual feature counts + histories
        with open(file_path, encoding="utf8") as f:
            for line in f:
...
class Feature2id:
    def __init__(self, feature_statistics: FeatureStatistics, threshold: int):
        """
        This function initializes the object of Feature2id, conversion from given feature count statistics and threshold
        parameter, into id representation of the features.
        """
    ...
    def prune_top_k_by_weight(self, weights: np.ndarray, K: int) -> None:
        ...
        # build a boolean mask of kept feature-indices
        mask = np.zeros(len(abs_w), dtype=bool)
        mask[topk] = True

        # rebuild feature_to_idx mapping
        ...
        # rebuild small_matrix for training
        rows, cols = [], []
