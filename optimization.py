...
def get_optimal_vector(statistics: FeatureStatistics,
                       ...
                       lam: float,
                       weights_path: str,
                       init_weights: np.ndarray = None) -> np.ndarray:
    if init_weights is None:
        w_0 = np.random.normal(0, 1, feature2id.n_total_features)
    else:
        # if the pruned feature set is smaller, truncate/pad
        w_0 = np.zeros(feature2id.n_total_features)
        w_0[:min(len(init_weights), feature2id.n_total_features)] = init_weights[:feature2id.n_total_features]
